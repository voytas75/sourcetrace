...
- The current LLM config boundary is intentionally split: `SourceTraceLlmConfig` owns only task-level routing (`model`, `temperature`, `max_output_tokens`), while provider bootstrap fields are declared separately in `LlmBootstrapConfig` as external env-var names and resolved only from the live process environment.
- For the currently wired runtime entrypoint (`build_llm_runtime(...)`), the Sourcetrace-owned task names are `claim_extraction`, `claim_normalization`, and `credibility_draft`; each gets its `model` from `SourceTraceLlmConfig.tasks[...]`.
- Provider bootstrap fields currently supported at the LiteLLM edge are `api_key`, `base_url`, and `api_version`; they are resolved via `resolve_llm_bootstrap_config(...)` and injected only inside `src/sourcetrace/llm/litellm_client.py`.
- For Azure / Microsoft Foundry GPT-5.x, the intended shape is LiteLLM `azure/...` model routing in `LlmTaskConfig.model`, with `SOURCETRACE_LLM_API_KEY`, `SOURCETRACE_LLM_BASE_URL`, and optionally `SOURCETRACE_LLM_API_VERSION` supplied by the external launcher rather than repo-owned config files.
...
- The LiteLLM adapter boundary now also owns the first real provider-bootstrap wiring helpers: `build_litellm_completion_caller(...)`, `build_litellm_text_generator(...)`, and `build_litellm_structured_generator(...)` inject resolved `api_key` / `base_url` / `api_version` values only at the adapter edge, keeping request models and application seams provider-neutral.
