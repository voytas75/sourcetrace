...
- The LiteLLM adapter layer now also includes first provider-bootstrap wiring helpers: `build_litellm_completion_caller(...)`, `build_litellm_text_generator(...)`, and `build_litellm_structured_generator(...)` consume `ResolvedLlmBootstrapConfig` and inject provider-facing bootstrap fields (`api_key`, `base_url`, `api_version`) only at the adapter edge.
