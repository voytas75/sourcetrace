...
FIXTURE_PATH = Path(__file__).resolve().parents[2] / "fixtures" / "v2" / "quality_regression_pack_v1.json"
...
def test_quality_regression_pack_v1_cases() -> None:
    for case in _load_cases():
        ...
        include_one_of = evaluation.get("must_include_one_of", [])
        if include_one_of:
            assert any(title in selected_titles for title in include_one_of)
            assert any(title in compiled_titles for title in include_one_of)
